...
git clone https://github.com/easymotion/vim-easymotion.git $HOME/.vim/pack/plugins/opt/easymotion
...
git clone https://github.com/vim-ruby/vim-ruby.git $HOME/.vim/pack/plugins/opt/vim-ruby
git clone https://github.com/tpope/vim-rails.git $HOME/.vim/pack/plugins/opt/rails
git clone https://github.com/tpope/vim-bundler.git $HOME/.vim/pack/plugins/opt/bundler
git clone https://github.com/tpope/vim-endwise.git $HOME/.vim/pack/plugins/opt/endwise
git clone https://github.com/vim-test/vim-test.git $HOME/.vim/pack/plugins/opt/vim-test
git clone https://github.com/dense-analysis/ale.git $HOME/.vim/pack/plugins/opt/ale
```
...
git clone https://github.com/vim-ruby/vim-ruby.git $HOME/.vim/pack/plugins/start/vim-ruby
git clone https://github.com/tpope/vim-rails.git $HOME/.vim/pack/plugins/start/rails
git clone https://github.com/tpope/vim-bundler.git $HOME/.vim/pack/plugins/start/bundler
...
[EasyMotion](https://github.com/easymotion/vim-easymotion) highlights possible targets on screen so you can jump there by typing a short label (fewer keypresses than repeating `w`/`j`).

The default prefix is `<Leader><Leader>` — with this config’s leader (`,`), press comma **twice**, then the motion key:

| Action                                                     | Keystrokes     |
| ---------------------------------------------------------- | -------------- |
| Jump to any word ahead / backward on screen                | `,,w` / `,,b`  |
| Same for a WORD (non-blank run)                            | `,,W` / `,,B`  |
| Jump to end of word forward / backward                     | `,,e` / `,,ge` |
| Search one character (any direction), then pick a label    | `,,s`          |
| Find character forward / backward on line (like `f` / `F`) | `,,f` / `,,F`  |
| Till character forward / backward (like `t` / `T`)         | `,,t` / `,,T`  |
| Jump to a line below / above                               | `,,j` / `,,k`  |
| Jump to next / previous search match on screen             | `,,n` / `,,N`  |

After the prefix, EasyMotion may wait for another key or show a prompt at the bottom. More motions (e.g. bidirectional word via `<Plug>(easymotion-bd-w)`) are documented in `:help easymotion`.
...
These shortcuts are configured in your `vimrc` with `vim-test`:

| Action              | Keystrokes |
| ------------------- | ---------- |
| run nearest test    | `,rt`      |
| run tests in file   | `,rf`      |
| run whole suite     | `,rs`      |
| rerun last test     | `,rl`      |
| jump to last test   | `,rv`      |

Useful Rails/Vim commands:

| Action                        | Command       |
| ----------------------------- | ------------- |
| open alternate file (spec/model/controller/view) | `:A`          |
| open related file in split    | `:AS`         |
| open related file in vsplit   | `:AV`         |
| run RuboCop via ALE (on save) | automatic     |
